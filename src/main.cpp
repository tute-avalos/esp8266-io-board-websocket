...
void onWebSocketEvent(AsyncWebSocket *server, AsyncWebSocketClient *client,
  ...
  if (type == WS_EVT_CONNECT) {
    Serial.println("Cliente conectado: " + String(client->id()));
  } else if (type == WS_EVT_DISCONNECT) {
    Serial.println("Cliente desconectado: " + String(client->id()));
...
void setup() {
  ...
  // Se lee el luxómetro cada 200 ms (una lectura en alta resolución tarda
  // ~120ms)
  pTasker.add(readBH1750, "bh", 200);
  // inicializa el TaskerManager
  pTasker.begin();
}
